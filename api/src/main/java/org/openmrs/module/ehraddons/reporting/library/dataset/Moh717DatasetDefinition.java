...
public class Moh717DatasetDefinition {
	...
	public DataSetDefinition constructMoh717Dataset() {
		...
		EhrReportingUtils.addRow(dsd, "OS", "OUTPATIENT SERVICES", ReportUtils.map(
				moh717IndicatorDefinition.getAllPatients(), indParams),
				EhrAddonUtils.getGeneralOutPatientFilters());

//TODO replace hardcoded mch  clinic
		EhrReportingUtils.addRow(dsd, "SPC", "SPECIAL CLINICS", ReportUtils.map(
				moh717IndicatorDefinition.getSpecialClinicPatients(EhrAddonsConstants.getConcept(EhrAddonsConstants._EhrAddOnConcepts.MCH_CLINIC).getConceptId()), indParams),
